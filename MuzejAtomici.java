 ...
 */
public class MuzejAtomici extends Application {
	private class Vodic {

		private AtomicInteger muzej;
		private AtomicInteger brStud = new AtomicInteger(0);
		private int grupa;

		public Vodic(AtomicInteger muzej, int grupa) {
			this.muzej = muzej;
			this.grupa = grupa;
		}
		...
		private void zapocniObilazak() throws InterruptedException {
			...
			do {
				...
				int oldV = muzej.get();
				ok = oldV == 0 || oldV == grupa;
				if (ok) {
					ok = muzej.compareAndSet(oldV, grupa);
				} else {
					Thread.yield();
				}
			} while (!ok);
			brStud.incrementAndGet();
		}
		...
		private void zavrsiObilazak() {
			...
			do {
				...
				if(ok) {
				ok = brStud.compareAndSet(oldV, newV);
				if (newV == 0)
					poslednji = true;
				} else {
					Thread.yield();
	...
	private AtomicInteger muzejAtomic = new AtomicInteger(0);

	private Vodic engleskiVodic = new Vodic(muzejAtomic, 1);
	private Vodic nemackiVodic = new Vodic(muzejAtomic, 2);
	private Vodic italijanskiVodic = new Vodic(muzejAtomic, 3);
	...
	protected final Operation obilazak = duration("5±2").text("Обилази").container(muzej).textAfter("Обишао")
			.update(this::azuriraj);
